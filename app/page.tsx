...
async function getNews(category?: string): Promise<NewsItem[]> {
  ...
  const qs   = category ? `?category=${encodeURIComponent(category)}&limit=20` : '?limit=20';
  try {
    const res = await fetch(`${base}/api/noticias${qs}`, { cache: 'no-store' });
    if (!res.ok) return [];
...
/* Sección "En Profundidad" — banda de categorías destacadas */
